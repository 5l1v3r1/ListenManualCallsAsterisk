<html >
	<head>
	 	...
	 	<style>
			...
			footer{bottom: 0;left: 0;width: 100%;background: #bababa;color: black;position: fixed;}
		</style>
	</head>
	...
	<script type="text/javascript">
							...
							<div class="hero-widget well well-sm">
								...
								<div class="icon">
									...
									<label class="text-muted">
										...
										<span id='timer' style="font-size:larger;color:#337ab7">${duration}</span>
									</label>
								</div>
							</div>
						</div>							
		...
		function updateTimer(){
			document.querySelectorAll('#timer').forEach(function(item){
				...
				var hh=Number(values[0]);
				var mm=Number(values[1]);
				var ss=Number(values[2]);
				if (ss<59) {
					ss+=1;
				}else{
					ss=0;
				...
				item.innerText=String(hh+':'+mm+':'+ss);;
			})
		}
